...
int main(int argc,char **argv)
{
    ...
    udem1 = LoadImagePgm(NAME_IMG_IN1, &length, &width);
    ...
    float** final_img = fmatrix_allocate_2d(4*length, 4*width);
    fmatrix_zero(4*length, 4*width, final_img);

    for (int i = 0; i < length; i++) {
	for (int j = 0; j < width; j++) {
	    final_img[i+length+(length-y)-30][j+width+x]= udem2[i][j];
    }
    }
    
    for (int i = 0; i < length; i++) {
	for (int j = 0; j < width; j++) {
	    final_img[i+length][j+width]= udem1[i][j];
    ...
    /* //Sauvegarde */
    SaveImagePgm(NAME_IMG_OUT1,final_img,4*length, 4*width);
